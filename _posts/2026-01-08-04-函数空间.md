...
本文档建立可积函数的空间理论。重点介绍L^1、L^2和L^∞空间，证明Riesz-Fischer定理（完备性）。详细研究这些空间的稠密子空间：简单函数、阶梯函数和光滑紧支函数。分析平移、伸缩等连续变换对函数的影响。介绍Holder不等式和Minkowski不等式，为L^p空间理论做准备。最后通过具体例子说明不同空间的关系和嵌入定理。
...
- **伸缩**：$S_\delta f(x) = f(\delta x)$，在 $\mathbb{R}^n$ 上 
 $$\|S_\delta f\|_{L^1}=\delta^{-n}\|f\|_{L^1}$$
