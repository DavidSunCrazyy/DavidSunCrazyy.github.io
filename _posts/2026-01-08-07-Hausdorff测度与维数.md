...
$$\mathcal{H}\_\delta^s(E) = \inf \{\sum_{j=1}^\infty \alpha(s)(\frac{\text{diam}(C_j)}{2})^s: E \subset \bigcup_{j=1}^\infty C_j, \text{diam}(C_j) \leq \delta \}$$
...
[**下一步：第八部分 拓扑空间测度**](/posts/real-analysis-08-拓扑空间测度/) 
