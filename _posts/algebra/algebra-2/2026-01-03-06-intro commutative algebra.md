...
::: proposition
$\Spec(\mathbb{C}[x,y])$ $$\Spec(\mathbb{C}[x,y])=\{(0)\}\cup \{(x-a,y-b)\}\cup \{(f):f\in \mathbb{C}[x,y], f \text{ irreducible}\}$$
:::
...
::: definition
设$R$是一个环，$R$的**素谱**是$R$的素理想的集合，记作$\Spec(R)$。
:::
...
::: proposition
若$\mathfrak{p}\in \Spec R$且$S\cap \mathfrak{p}=\emptyset$，则$S^{-1}\mathfrak{p}$在$S^{-1}R$为素理想，并且$\varphi^{-1}(S^{-1}\mathfrak{p})=\mathfrak{p}$
:::
...
::: proposition
设$S$是$R$的乘法封闭子集，$S^{-1}R$的素理想一一对应于$R$的素理想$P$满足$P\cap S=\emptyset$。 $$\Spec S^{-1}R \rightarrow \{\mathfrak{p}\in \Spec R:\mathfrak{p}\cap S=\emptyset\}$$
:::
