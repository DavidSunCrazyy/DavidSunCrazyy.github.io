...
取 $z = -\frac{\omega}{2}$,得到 $c\_\omega = \wp(\frac{\omega}{2}) - \wp(-\frac{\omega}{2}) = 0$ $\mathrm{Im}plies$ $\wp$ 是椭圆函数.
...
$X\subset \mathbb{C}^2$ 是一个连通子集,可以通过添加一个点 $\infty$ 来紧化. 令 $x = \frac{1}{w}$,则 $$y^2  = 4(\frac{1}{w}-e\_1) (\frac{1}{w}-e\_2) (\frac{1}{w}-e\_3) \mathrm{Im}plies (y^2 \cdot w^2) = 4\cdot w (1-e\_1 w)(1-e\_2 w)(1-e\_3 w)$$ 记 $\tilde{y} = yw^2$,则 $y = \frac{\tilde{y}}{w^2}$,我们看到对于 $w=0 \iff x=\infty$,恰好有一个解 $\tilde{y}=0$.
...
补集 $\mathbb{CP}^2 \setminus \mathbb{C}^2$ 是 $\mathbb{CP}^1$: $l \in \mathbb{CP}^2 \setminus \mathbb{C}^2 \mathrm{Im}plies l = \mathbb{C} (z\_1,z\_2,z\_3)$ 且 $z\_3 = 0$ $\mathrm{Im}plies$ $l = \mathbb{C} (z\_1,z\_2,0) = \mathbb{C}(z\_1,z\_2) \in \mathbb{CP}^1$.

像 $\mathbb{CP}^1$ 一样,我们可以使用其他坐标来计算 $\mathbb{CP}^2 \setminus \mathbb{C}^2$ 附近,例如通过 $(\tilde{z},\tilde{w}) \mapsto [1,\tilde{z},\tilde{w}]$ 或 $(\hat{z},\hat{w}) \mapsto [\hat{z},1,\hat{w}]$,其中 $(1,\tilde{z},\tilde{w}) = \lambda(1,z,w) \mathrm{Im}plies \tilde{z} = \frac{w}{z}, \tilde{w} = \frac{1}{z}$.
