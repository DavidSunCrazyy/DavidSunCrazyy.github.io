...
note: true
layout: post
title: "Real Analysis 微分理论"
permalink: /posts/real-analysis-06-微分理论/
categories: real-analysis
tags: [real-analysis, differentiation, radon-nikodym, vitali-covering, absolute-continuity, bounded-variation]
use_math: true
---
...
**定理**：设 $E \subset \mathbb{R}^d$ 满足 $m^*(E) < \infty$，$\mathcal{B}$ 是 $E$ 的 Vitali 覆盖，则对任意 $\delta > 0$，存在有限个两两不交的 $B_j \in \mathcal{B}$ 使得：
...
$$\mathcal{M}(f)(x) = \sup_{B : x \in B} \oint_B \|f(y)\| dy$$
...
$$\lim_{\|B\| \to 0, x \in B} \oint_B f(y) dy = f(x)$$
...
**等价刻画**：
1. $f \in AC(I)$
2. $f'$ 几乎处处存在且可积，且 $f(x) - f(a) = \int_a^x f'(t) dt$
3. 存在 $g \in L^1([a,b])$ 使得 $f(x) = f(a) + \int_a^x g(t) dt$
...
若 $f$ 单调递增，不假设连续性，则 $f = f_a + f_c + f_j$，其中 $f_j$ 是跳跃函数.。
因此，任意 $f \in BV$ 都可唯一分解为$f = f_a + f_c + f_j$.
...
[**下一步：第七部分 Hausdorff测度与维数**](/posts/real-analysis-07-Hausdorff测度与维数/)
