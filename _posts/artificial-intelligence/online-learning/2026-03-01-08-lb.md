...
tags: [online-learning, linear-bandits, optimism-in-face-of-uncertainty, confidence-ellipsoids, linear-regression]
use_math: true
---

这篇文章系统介绍了**线性老虎机（Linear Bandits）**的算法与理论。在线性老虎机中，每个动作由一个上下文向量表示，期望奖励为向量与未知参数 $\vec{\theta}$ 的内积，目标是最小化遗憾。文章首先推导了基于岭回归（OLS with regularization）的参数估计和置信区域构造，分析了**LinUCB 算法**：在每轮选取「估计奖励 + 置信上界」最大的动作，利用椭球势能引理（Elliptical Potential Lemma）证明了 $O(d\sqrt{T\log T \log(nT)})$ 的遗憾上界。由于 LinUCB 存在噪声与动作选择的依赖性问题，文章进一步介绍了 **SupLinUCB 算法**——通过将历史数据按层划分、层层筛选动作，确保了噪声独立性，从而严格证明了遗憾界。对于**无穷多臂**场景，文章讨论了 $\varepsilon$-覆盖方法和自归一化尾界（Self-Normalized Tail Bound）技术，后者可处理自适应对手并将遗憾优化至 $O(d\sqrt{T}\log T)$。最后，文章还覆盖了**稀疏线性老虎机**的扩展，给出了基于 $s$-稀疏约束的 $O(\sqrt{sd\log(dT)\log T})$ 遗憾界，以及用于鞅差分的自适应 Azuma 不等式这一关键技术工具。
