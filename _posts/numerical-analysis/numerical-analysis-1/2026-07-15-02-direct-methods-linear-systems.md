...
本文介绍线性代数方程组的直接解法，包括 Gauss 消去法（顺序消去与列选主元）、LU 分解（Doolittle 方法）、对称正定矩阵的 Cholesky 分解、带状矩阵方程组的追赶法及 Parlett-Reid 算法等。
...
$$
l_{1} = (0, l_{21}, \dots, l_{n1})^{T} \in \mathbb{R}^{n}
$$

$$
L_{1} = I + l_{1} e_{1}^{T} = \left[\begin{array}{cccc} 1 & & & \\ l_{21} & 1 & & \\ \vdots & & \ddots & \\ l_{n1} & & & 1 \end{array}\right]
$$

$$
\text{则 } L_{1}^{-1} = I - l_{1} e_{1}^{T} = \left[\begin{array}{cccc} 1 & & & \\ -l_{21} & 1 & & \\ \vdots & & \ddots & \\ -l_{n1} & & & 1 \end{array}\right]
$$
...
设 $A = [a_{ij}] \in \mathbb{R}^{n \times n}$ ， $\Delta_{i} \neq 0$ ， $i = 1, 2, \cdots, n$ 则存在
单位 下三角矩阵 $L = [l_{ij}]$ ，上三角矩阵 $U = [u_{ij}]$ 使得 $A = LU$ 即
...
由归纳假设 存在唯一的对角元素为正数的
下三角矩阵 $\widetilde{L}$ 使得 $\widetilde{A}_{22} = \widetilde{L} \widetilde{L}^{T}$ 令
